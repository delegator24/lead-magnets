...
(function(){
 ...
 // С мероприятия (&ev=1 в ссылке из событийного теста) возврат — ЧИСТЫЙ чат:
 // человек уже в событийной воронке, метка materialtg увела бы его в тетрадную
 // (поймано Юлей на живом тесте 21.08).
 var событие=(new URLSearchParams(location.search).get('ev')||'')==='1';
 var ссылкаБота=(канал==='max'?'https://max.ru/id2634102694_bot':'https://t.me/Delegator24_bot')
                +(событие?'':'?start='+меткаВозврата);
